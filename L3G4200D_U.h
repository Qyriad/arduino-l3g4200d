 ...
 * @file L3G4200D_U.h
 *
 * @mainpage ST @htmlonly L3G4200D @endhtmlonly Gyroscope Unified Sensor Library.
 *
 * @section intro_sec Introduction
 *
 * This is a library for the ST @htmlonly L3G4200D @endhtmlonly gyroscope sensor, found on some boards
 * like the Digilent PmodGYRO or the Waveshare L3G4200D breakout.
 *
 ...
 * [Unified Sensor API](https://learn.adafruit.com/using-the-adafruit-unified-sensor-driver/how-does-it-work).
 *
...
/*! @brief The address of CTRL_REG3, which is used for configuring the electrical
 * characteristics of the pins on the chip.
 *
 * @see CTRL3.
 */
#define REG_CTRL_3    (0x22)

/*! @brief The address of CTRL_REG4, which is used for setting the gyroscope range.
 *
 * @see CTRL4.
 */
...
/*! @brief The chip ID constant value of [REG_WHO_AM_I](@ref REG_WHO_AM_I): `0xd3`.
 *
...
/*! @brief REG_CTRL_2 value for filtering based on the data rate divided by 1000. */
#define CTRL2_HIGH_PASS_DIV_1000   (0b0110 << 0)

/*! @brief REG_CTRL_2 value for filtering based on the data rate divided by 2000. */
#define CTRL2_HIGH_PASS_DIV_2000   (0b0111 << 0)

/*! @brief REG_CTRL_2 value for filtering based on the data rate divided by 5000. */
#define CTRL2_HIGH_PASS_DIV_5000   (0b1000 << 0)

/*! @brief REG_CTRL_2 value for filtering based on the data rate divided by 10000. */
 ...
 * @ingroup registers
 *
 * @brief Values for @ref REG_CTRL_4, which is used for setting the gyroscope range.
 *
 * @{
 */
...
class L3G4200D_Unified : public Adafruit_Sensor {
  ...
  public:
     ...
     * @endparblock
     *
     * @param range One of the ::gyroRange_t values to use for this gyroscope. Defaults to 4.36 rad/s.
     * @param spi
     * @parblock
     ...
     * @endparblock
     *
     * @returns True if this sensor was successfully activated, false if it was not.
     * If false, you can use @ref enableDebugLogging to potentially get information
     * on why logged in your serial console.
     */
    bool begin(int spiChipSelect, gyroRange_t range = GYRO_RANGE_4_DOT_36_RAD_PER_SEC, SPIClass &spi = SPI);
     ...
     *
     * @returns True if this sensor was successfully read from, false if it was not.
     */
    bool getEvent(sensors_event_t *event);

    /*! @brief The Unified Sensor API method to get information about this sensor.
     * @param sensor [out] A pointer to a sensor_t object for this method to populate
     * with information about this sensor.
     */
     ...
     * One of the values of [register addresses](@ref reg_addresses).
     *
     * @param newValue The raw value to write to the register specified in @p regAddress.
     */
  ...
  private:
    ...
    /*! @brief Reads samples for the X, Y, and Z axes all at once as one transaction. */
    ...
    /*! @brief Starts a transaction, writes to a register, and ends the transaction. */
    ...
    /*! @brief Logs if enabled with @ref enableDebugLogging, prepending the sensor ID first. */
    void debugLog(const char str[]);

    /*! @brief Logs if enabled with @ref enableDebugLogging, prepending the sensor ID first. */
    void debugLog(int val);

    /*! @brief Logs if enabled with @ref enableDebugLogging, without prepending the sensor ID first. */
